...
class EditUserViewModel: BaseViewModel<EditUserViewModel.State, EditUserViewModel.Action, Never> {
    ...
    struct State: AnyState {
        ...
        public fileprivate(set) var isNameValid: Bool = true
        //User
    ...
    override func action(_ action: Action) {
        ...
        case .updateUser:
            if state.isNameValid {
                realmManager.updateUser(user: state.user!, name: state.name, image: state.image, phone: state.phone, email: state.email, country: state.country, city: state.city, street: state.street, number: state.number)
                state.showedScreen = nil
                state.showedScreen = .back
            }
        case .deleteUser:
            realmManager.deleteUser(name: state.user!.name)
            state.showedScreen = nil
            state.showedScreen = .back
            //Image
